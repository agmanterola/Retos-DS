...
### Ránking de secciones: Solución

Lo primero será cargar el dataset:
```{r}
dataset <- read.csv("./dat/sscc_madrid_data.csv")
```

Para cada campaña hay que hacer básicamente los mismos cálculos. Eso sí, usando diferentes indicadores y en algún caso, utilizando más de un indicador (indicadores compuestos).
Tal y como está organizada la información:

Camapaña|Tipo_indicador|Indicador(es)            
--------|--------------|-------------------------
Ni Hao  |Simple        |total_china              
Joquers |Simple        |estudiantes_de_grado     
Yayofón |Simple        |mujeres_mas_65           
Huver   |Compuesto     |tramo & total_entre_35_50

Lo primero será filtrar el dataset para generar un dataset acorde a las necesidades. Sobre éste, habrá que crear los nuevo indicadores y hacer el ránking.

#### Ránking: Creación de datasets por campaña

Tomando como ejemplo la campaña Ni Hao, sólo nos interesa conseguir la gente nacida en china, el total de población y el total de viviendas por sección censal. No hay filtros, sólo selección de columnas.
...
```{r}
# Dataset para la campaña de Joquers
joquers<- dataset %>%
            select(id, municipio, distrito, seccion, total_poblacion,estudiantes_de_grado, viviendas)

# Dataset para la camapaña de Yayofón
yayofon <- dataset %>%
            select(id, municipio, distrito, seccion, total_poblacion,mujeres_mas_65, viviendas)
```

Para el caso de la camapaña de HUVER, habría que considerar 2 indicadores, la edad comprendida entre 35 y 50 años y que su nivel de estudios sea de 3º grado (diplomatura, licenciatura o máster). Además, está el tema del nivel de renta, pero en vez de tomarlo como indicador, se filtrará el número de observaciones por tramos > a 7.
